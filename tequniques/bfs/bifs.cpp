...
//bfs
//queは始点の情報をいれておく.
//入れるときに距離を記録しておくと良い. (別に取り出すときでもいい)
//queから取り出した要素から行ける要素をpushしてく.
//O(E)
int bfs(vector<vector<char>> v, int r, int c, int sy, int sx, int gy, int gx) {
  vvi w(r, vi(c));
  queue<pi> q;
  ...
  while (!q.empty()) {
    ...
    //r
    if (x < r - 1 && v[x + 1][y] != '#' && u[x + 1][y] == 0) {
      q.push(make_pair(x + 1, y));
      w[x + 1][y] = w[x][y] + 1;
      u[x + 1][y] = 1;
    }
    //l
    if (0 < x && v[x - 1][y] != '#' && u[x - 1][y] == 0) {
      q.push(make_pair(x - 1, y));
      w[x - 1][y] = w[x][y] + 1;
      u[x - 1][y] = 1;
    }
    //a
    if (0 < y && v[x][y - 1] != '#' && u[x][y - 1] == 0) {
      q.push(make_pair(x, y - 1));
      w[x][y - 1] = w[x][y] + 1;
      u[x][y - 1] = 1;
    }
    //b
    if (y < c - 1 && v[x][y + 1] != '#' && u[x][y + 1] == 0) {
      q.push(make_pair(x, y + 1));
      w[x][y + 1] = w[x][y] + 1;
      u[x][y + 1] = 1;
    }
...
int main() {
  ...
  cout << bfs(v, R, C, sy - 1, sx - 1, gy - 1, gx - 1) << endl;
}
...
j
 ...
 は #である。また、スタート地点とゴール地点は必ず空きマスであり、スタート地点からゴール地点へは、空きマスを辿って必ずたどり着ける。
出力
スタート地点からゴール地点に移動する最小手数を
1
 行に出力せよ。最後に改行を忘れないこと。
入力例1
Copy
...
図8のように進めば
10
 手でたどり着くことが進むことができます(※Sはスタート、Gはゴールをあらわす)。
...
94
94
もはやただの広い空間であり、迷路と呼んで良いのかは分からないがこの問題でこのような盤面も迷路として扱う。兎にも角にも、スタートからゴールへの最短手数は
94
 である。
 */
